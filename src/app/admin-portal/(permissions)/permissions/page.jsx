...
import { getAllPermissions, deletePermission } from "@/utils/api";
...
export default function PermissionManagement() {
  ...
  useEffect(() => {
    const fetchPermissions = async () => {
      ...
      try {
        ...
        setTotalPermissions(response.data.meta.total);
      } catch (err) {
        const errorMsg =
   ...
   if (!hasAdminAccess) {
    ...
    const timer = setTimeout(() => fetchPermissions(), 300);
    return () => clearTimeout(timer);
  }, [pagination.pageIndex, pagination.pageSize, refreshKey]);

  const handleDelete = (id, permission) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (permissions.length === 1 && pagination.pageIndex > 0) {
        setPagination((prev) => ({
          ...prev,
          ...
          return (
            <div className="min-w-0">
              ...
              {permission.description && (
                <div className="text-xs text-gray-500 mt-0.5 max-w-2xl">
                  {permission.description}
                </div>
              )}
            </div>
          ...
          <div className="flex items-center space-x-1.5">
            {hasPermission("Permission - Update") && (
              <button
                onClick={() =>
                  router.push(
                    `/admin-portal/new-permission?editId=${row.original.id}`,
                  )
                }
  ...
  const table = useReactTable({
    ...
    pageCount: Math.ceil(totalPermissions / pagination.pageSize),
  ...
  const end = Math.min(start + permissions.length - 1, totalPermissions);
          ...
          <div className="p-6">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
              ...
              {hasPermission("Permission - Create") && (
                <button
                  onClick={() => router.push("/admin-portal/new-permission")}
            ...
            <div className="mb-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div className="space-y-1 text-sm text-gray-600">
                  <p>• This permission will be removed from all roles</p>
                  <p>
                    • Users will lose access rights associated with this
                    permission
                  </p>
                </div>
              </div>
